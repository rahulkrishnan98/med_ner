...
class BiLSTMCRF(nn.Module):
    def __init__(self, sent_vocab, tag_vocab, dropout_rate=0.5, embed_size=256, hidden_size=256):
        ...
        # A parameter is different from a tensor since it becomes part of the model params
    ...
    def encode(self, sentences, sent_lengths):
        ...
        """
        # ignore the padded inputs when calculating gradients for backprop
...
if __name__ == "__main__":
    ...
    utils.createDir(
        [config["SENT_VOCAB_PATH"], config["model_save_path"]]
    )
    #step_1: load vocab
    ...
    #step_3: Choose device- cpu or cuda
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    #step_4: Model life cycle (check if re-load is successful)
    model = BiLSTMCRF(sent_vocab, tag_vocab)
    print("Completed Model init")
